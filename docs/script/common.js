...
$(document).ready(function() {
    ...
    $('button[onclick="addSearchKey()"]').click(function() {
        bannerCount++;

        var newRow = `
            <tr>
                <th scope="col">` + bannerCount + `</th>
                <td class="banner_setting">
                    <div class="d-flex align-item-center gap10">
                        <div class="input_check">
                            <input type="checkbox" class="" value="" id="check_new_${bannerCount}" name="check_01">
                            <label for="check_new_${bannerCount}"></label>
                        </div>
                        <div class="banner_area"></div>
                        <button type="button" class="btn03 btn-modify"><span>수정</span></button>
                        <button type="button" class="btn03 btn-hidden"><span>숨김</span></button>
                        <button type="button" class="btn03 btn-show"><span>노출</span></button>
                    </div>
                </td>
            </tr>
        `;
        $('.input_table tbody').append(newRow);
    });

    // 삭제 버튼 클릭 시 선택된 tr 삭제
    $('button[onclick="delSearchKey()"]').click(function() {
        $('input[name="check_01"]:checked').each(function() {
            $(this).closest('tr').remove();  // 체크된 tr을 삭제

            bannerCount = 0; 
            $('.input_table tbody tr').each(function() {
                bannerCount++;
                $(this).find('th').text(bannerCount); 
            });
        });
    });
});
...
$(document).ready(function() {
    ...
    $(".btn05").click(function() {
        $(this).closest("ul").find(".mb_table02").stop(true, true).slideToggle(400); // Adjust the duration for smoothness
        $(".btn05").not(this).removeClass("active");
...
$(document).ready(function() {
    $('.hamburger').click(function(e) {
      e.preventDefault();
      $('.section_left').toggleClass('active');
    });
  });
